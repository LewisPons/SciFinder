def extract_authors(flat_dict):
    ...
    while True:
        ...
        if (
            fore_name_key not in flat_dict
            and last_name_key not in flat_dict
            and collective_name_key not in flat_dict
        ):
            break
...
def consolidate_author_keys(flat_dict):
    ...
    keys_to_remove = [
        key for key in flat_dict if key.startswith("abstract_authors_list_Author_")
    ]
